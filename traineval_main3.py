...
from utils.dataloader3 import dataloader
...
from EPcode.SARNNSim3 import SARNNM # as SARNNMEP13
...
if __name__ == '__main__':
    ...
    # realpath1 = r"D:\Project\Data\MSTAR\SLICYSim\CSV\30_DEG"
    # realpath2 = r"D:\Project\Data\MSTAR\SLICYSim\CSV\15_DEG"
    realpath1 = r"D:\Project\Data\MSTAR\T72WSim\CSV\17A05"
    realpath2 = r"D:\Project\Data\MSTAR\T72WSim\CSV\15A05"
    # ==============================================
    # 载入数据集
    batch_size = 4
    resolutioin = 0.5
    img_size = int(28.2/resolutioin*2.5) #SLICY 54*2 T72 138
    train_loader = dataloader(realpath1, realpath2, img_size, batch_size, device)

    # ==============================================
    distance = 5 # distance from camera to the object
    # SlantPixSpacing = 0.2 / 2
    SlantPixSpacing = resolutioin/28.2
    ...
    # ==============================================
    # ==============================================
    savepath = r'.\logs\Result\EP'
    logpath = r'.\logs\runs\EP'
    # 建立模型
    EPNum = 10
    model1 = SARNNM(obj_filename=obj_filename, simsetting = simsetting ,
                    EPNum = EPNum, savepath=savepath, logpath = logpath,device=device)

    EPNum = 0
    model2 = SARNNM(obj_filename=obj_filename, simsetting = simsetting ,
                    EPNum = EPNum, savepath=savepath, logpath = logpath,device=device)
    ...
    model1.optimizer = torch.optim.SGD(paramsets, lr=1.0, momentum=0.9)
    ...
    model2.optimizer = torch.optim.SGD(paramsets, lr=1.0, momentum=0.9)
    ...
    for epoch in loop:
        ...
        for trainindex, sample in enumerate(train_loader):  ####
            ...
            if trainindex > 5:
                break
            # break #######
        model1.writer.add_scalar('sum loss', train_loss1 / len(train_loader), global_step=epoch + 1)
        model2.writer.add_scalar('sum loss', train_loss2 / len(train_loader), global_step=epoch + 1)
        ...
        if (epoch > 10) and (train_loss_old1 > (train_loss1 / len(train_loader))) and (epoch%10 == 0):
            model1.msave(epoch + 1)
            # train_loss_old1 = train_loss1 / len(train_loader)
            train_loss_old1 = train_loss1 / len(train_loader)
        if (epoch > 10) and (train_loss_old2 > (train_loss2 / len(train_loader))) and (epoch%10 == 0):
            model2.msave(epoch + 1)
            train_loss_old2 = train_loss2 / len(train_loader)
        ...
        desc = f"train_loss: {train_loss1 / 4}"
    ...
    plot_losses(model1.losses)
    plt.show()

    plot_losses(model2.losses)
    plt.show()
