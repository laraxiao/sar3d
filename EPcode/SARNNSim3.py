...
class SimPix(nn.Module):
    ...
    def forward(self, input):
        ...
        output = input # self.Conv1(input) # 仅使用卷积核
...
class SARNNM:
    ...
    def __init__(self, obj_filename, simsetting, device,
                 EPNum = 0, savepath= r".\results", logpath = r'.\logs\runs\EP' ):
        ...
        self.SLICY_mesh = ico_sphere(5, device)
        self.SLICY_mesh.scale_verts_((0.7))
        ...
        obj_filename = r'D:\Project\pytorch3d-030\pytorch3d-master\docs\tutorials\data\B520.obj'
        mesh = load_objs_as_meshes([obj_filename], device=device)
        t_shape = mesh.verts_packed().shape
        t_rgb = torch.full([1, t_shape[0], 3], 0.665, device=device)
        mesh.textures = TexturesVertex(verts_features=t_rgb)

        # T72W 相关设置
        # verts = mesh.verts_packed()  # (N 3)
        # scale = max((verts).max(0)[0]) - min((verts).min(0)[0])
        # mesh.scale_verts_((1.0 / float(scale)));
        # mesh.scale_verts_(9.53);
        ...
        self.optimizer =  torch.optim.SGD(paramsets, lr=1.0, momentum=0.9)
    ...
    def sarimgnnsim(self,sample, meshes):
        ...
        yfactor = 2
        ysize = self.SlantPixSpacing * self.imagesize / 2 * torch.tan(elev * deg) * yfactor
        # ysize = xsize
        cameras = FoVOrthographicCameras(
            znear=0.1, zfar=distance + 40.0,
        ...
        sigma = 1e-4
        imagesize = (int(imagesize*yfactor), imagesize) #(H,W)
        ...
        sarsilimgs = silimgs.unsqueeze(1)

        # imgt = sarsilimgs[0, 0, ...]
        # imgplot(torch.tanh(imgt))
        # plt.show()
        # a = imgt.detach().cpu().numpy()

        return sarimgs,sarsilimgs

    def sarimgnnsim2(self,sample, meshes):
        device = self.device
        distance = self.distance
        deg = self.deg
        imagesize = self.imagesize
        ...
        # cameras = OpenGLPerspectiveCameras(device=device, R=R_goal, T=T_goal)
        # xsize = self.SlantPixSpacing * self.imagesize / 2
        xsize = self.SlantPixSpacing * self.imagesize
        xsize = torch.tensor(xsize, dtype=torch.float32, device=device).expand(batch_size)
        # ysize = self.SlantPixSpacing * self.imagesize / 2 * torch.tan(elev * deg) * 1.4
        ysize = self.SlantPixSpacing * self.imagesize * torch.tan(elev * deg) * 1.4
        # ysize = xsize
        cameras = FoVOrthographicCameras(
            znear=0.1, zfar=distance + 40.0,
            max_y=ysize, min_y=-ysize,
    ...
    def coreprocesses(self, sample):
        ...
        _,pre_sarsilimgs = self.sarimgnnsim(sample,new_src_mesh)

        with torch.no_grad():
            target_mesh = self.Target_mesh
            _,tar_sarsilimgs = self.sarimgnnsim(sample,target_mesh)
        ...
        # return imgreg,refreg
        return tar_sarsilimgs,pre_sarsilimgs
    ...
    def mtrain(self, sample, epoch = 0):
        ...
        tar_sarsilimgs,pre_sarsilimgs = self.coreprocesses(sample)
        # 计算损失函数值
        ...
        # tar_sarsilimgs = F.max_pool2d(tar_sarsilimgs, kernel_size=2, stride=2, padding=0)
        # pre_sarsilimgs = F.max_pool2d(pre_sarsilimgs, kernel_size=2, stride=2, padding=0)

        loss_rgb = torch.tensor(0.0, device=self.device) #self.criterionL2(sarimgs, Target)
        loss["rgb"] += loss_rgb

        # a = tar_sarsilimgs.detach().cpu().numpy()

        loss_silhouette = self.criterionL2(pre_sarsilimgs, tar_sarsilimgs)
    ...
    def mtest( self, sample, epoch = 0):
        ...
        with torch.no_grad():
            ...
            sarimgs,sarsilimgs= self.coreprocesses(sample)

            # 计算损失函数值
            loss1 = self.criterionL2(sarimgs,Target)
            loss = loss1
