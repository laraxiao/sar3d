...
class SARNNM:
    ...
    def sarimgnnsim(self, sample, meshes):  # sample is the ground truth table( a sar image from specific view angle, and meshes is from the new 3D model, now have to generate the 2d figure of this new 3D model from the same angle as the as the goundtruth looking at angle, so that it can be compared and do loss calculation)
        ...
        sarimgs_original = sarimgs.clone()
    
    # Create enhanced version for visualization, normalization 
        sarimgs_enhanced = sarimgs.clone()
        for i in range(sarimgs_enhanced.shape[0]):
            min_val = sarimgs_enhanced[i].min()
            max_val = sarimgs_enhanced[i].max()
        if max_val > min_val:  # Prevent division by zero
            sarimgs_enhanced[i] = (sarimgs_enhanced[i] - min_val) / (max_val - min_val)

# Use enhanced version for visualization only
        self.sarimgs = sarimgs_enhanced
    ...
    def coreprocesses(self, sample):
        ...
        Targetsilimg = (Targetimg<0.005)*1.0  #the real SAR silhouette mask(where the pixel intensity is very low)
